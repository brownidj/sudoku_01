...
class ImmediateAnimalAssetService extends AnimalAssetService {
  @override
...
Future<void> _pumpLaunch(
  ...
  AnimalAssetService? assetService,
}) async {
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('Play'), findsOneWidget);
      expect(find.text('SuDoKu Fresh'), findsOneWidget);
      service.complete();
      await tester.pumpAndSettle();
  ...
  testWidgets('Play starts the game on Android', (tester) async {
    try {
      ...
      await _pumpLaunch(
        tester,
        controller,
        assetService: ImmediateAnimalAssetService(),
      );

      await tester.tap(
        find.byKey(const ValueKey<String>('launch-play-button')),
      );
      await tester.pumpAndSettle();
